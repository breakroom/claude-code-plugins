---
name: circleci
description: Patterns for invoking the CircleCI CLI (circleci) from agents. Use when investigating CI on a project that uses CircleCI — failed jobs, pipeline runs, step output, workflows, env vars — or whenever the `circleci` command is needed. Covers structured output, project and org targeting, and the `circleci api` fallback.
---
...
The `circleci` CLI must be installed and authenticated. If `circleci` is not
found, give the user installation instructions but do not install it yourself:

```bash
# If the older stable CLI is installed, remove it first
brew uninstall circleci
brew install circleci-public/circleci/circleci@next
```

Authenticate with `circleci auth login`, or set `CIRCLECI_TOKEN`. Note that
these commands require v1.x of the CLI (the agent-friendly rewrite) — the older
stable CLI does not have `circleci run`, `circleci job`, or `--json`.
